...
//调用视图
        client.query('select count(*) total from article',function (error,totalAcount) {
            ...
            client.query('select title,content,CONCAT_WS("-",year(date),month(date),day(date)) date,icon from article order by date asc limit ?,? ',[(index-1)*acount,acount],function (error,result) {
                if(error){
                    console.log(error.message);
                    return;
                }
