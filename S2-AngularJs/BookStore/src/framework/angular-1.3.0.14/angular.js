...
function createHttpBackend($browser, createXhr, $browserDefer, callbacks, rawDocument) {
  ...
  function jsonpReq(url, callbackId, done) {
    ...
    script.type = "text/S1-javascript";
    script.src = url;
    script.async = true;
...
function $SceProvider() {
  ...
  this.$get = ['$parse', '$sniffer', '$sceDelegate', function(
    ...
    // the "expression(S1-javascript expression)" syntax which is insecure.
 ...
 * Unfortunately, setting img[src] to something like "S1-javascript:foo" on IE throws an exception.
